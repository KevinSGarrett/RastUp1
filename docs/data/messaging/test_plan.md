...
| Workspace data sources | GraphQL execution vs Safe-Mode stub fallback for inbox/thread hydrators | `tests/frontend/messaging/dataSources.test.mjs` verifies shared data source helpers honour provided fetch implementations, propagate query variables, and gracefully return stub payloads on failures. |
