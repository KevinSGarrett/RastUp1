...
| Workspace data sources | GraphQL execution vs Safe-Mode stub fallback for inbox/thread hydrators, mutations, and subscriptions | `tests/frontend/messaging/dataSources.test.mjs` verifies shared data source helpers honour provided fetch implementations, propagate query variables, emit GraphQL mutations (send/read/request), surface stub fallbacks, and expose no-op subscription defaults until AppSync wiring is ready. |
