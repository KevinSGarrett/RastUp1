...
| Upload manager | Client-side lifecycle (request → signed → upload → scanning → ready), failure recovery, TTL pruning | Mock server updates for antivirus results, verify attachment linking/metadata retention, and confirm timeout transitions set `errorCode: UPLOAD_STATUS_TIMEOUT`. |
...
| Messaging controller | Combined inbox/thread/notification orchestration, optimistic lifecycle, subscription events | Unit tests validating unread sync, optimistic send resolution, message request acceptance, filter-aware `selectInboxThreads`, action card analytics, digest releases, thread management helpers (pin/archive/mute), and moderation dual-approval handling via `submitModerationQueueDecision`. |
| Messaging client | GraphQL hydration/subscription bridge, optimistic mutation handshake, request mutations, upload pipeline | Unit tests verifying `createMessagingClient` refreshes inbox, hydrates threads, applies mapped events, resolves/flags optimistic sends, invokes request mutations with re-sync on failure, orchestrates `prepareUpload`/`cancelUpload` lifecycles, polls `getUploadStatus` until terminal states (including timeouts), exercises thread pin/archive/mute mutations, and hydrates/moderates queue cases across SSR + runtime (decide/resolve/remove paths with/without GraphQL overrides). |
| React bindings | `MessagingProvider` lifecycle, hook subscriptions, store updates via controller events, notification actions surface | Use a lightweight React shim to assert auto-subscribe/cleanup behaviour, that `useInboxThreads`, `useThread`, `useInboxSummary` reflect controller changes, and that `useMessagingActions` exposes moderation helpers (`submitModerationDecision`, `resolveModerationQueueCase`, `removeModerationQueueCase`) alongside `pinThread`/`archiveThread`/`muteThread` and notification queue operations. |
| Moderation queue | Message/thread reporting reducers, moderation queue transitions, inbox block filters | `tests/frontend/messaging/moderation_queue.test.mjs` exercises queue reducers (`enqueueCase`, `updateCase`, `submitDecision`, `resolveCase`, `removeCase`), selectors, stats (`awaitingSecond`), and dual-approval pathways. Controller/client/react binding suites cover reporting helpers, SSR + client queue hydration, decide/resolve/remove pathways, and ensure blocked threads respect filter options. |
| Next.js adapter | Server prefetch + hydration helpers, provider prop wiring | Unit tests cover `createMessagingNextAdapter.prefetch`, `createProviderProps`, and `createRuntime` to ensure inbox/thread payloads hydrate correctly, moderation queue cases serialize for SSR, and runtime clients reuse fetch/mutation hooks. |
| Workspace data sources | GraphQL execution vs Safe-Mode stub fallback for inbox/thread hydrators, mutations, subscriptions, and uploads | `tests/frontend/messaging/dataSources.test.mjs` verifies shared data source helpers honour provided fetch implementations, propagate query variables, emit GraphQL mutations (send/read/request/thread management), surface stub fallbacks, expose no-op subscription defaults, return stub upload session/status data until AppSync wiring is ready, and fetch moderation queue cases when GraphQL endpoints exist. |
