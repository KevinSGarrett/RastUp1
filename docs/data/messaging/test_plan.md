...
| Messaging controller | Combined inbox/thread/notification orchestration, optimistic lifecycle, subscription events | Unit tests validating unread sync, optimistic send resolution, message request acceptance, filter-aware `selectInboxThreads`, action card analytics, digest releases, and thread management helpers (pin/archive/mute). |
| Messaging client | GraphQL hydration/subscription bridge, optimistic mutation handshake, request mutations, upload pipeline | Unit tests verifying `createMessagingClient` refreshes inbox, hydrates threads, applies mapped events, resolves/flags optimistic sends, invokes request mutations with re-sync on failure, orchestrates `prepareUpload`/`cancelUpload` lifecycles, polls `getUploadStatus` until terminal states, emits timeouts when scans stall, and exercises thread pin/archive/mute mutations with/without GraphQL overrides. |
...
| Moderation queue | Message/thread reporting reducers, moderation queue transitions, inbox block filters | `tests/frontend/messaging/moderation_queue.test.mjs` exercises queue reducers (`enqueueCase`, `updateCase`, `resolveCase`, `removeCase`, selectors, stats). Controller/client/react binding suites cover reporting helpers, queue hydration, resolve/remove pathways, and ensure blocked threads respect filter options. |
